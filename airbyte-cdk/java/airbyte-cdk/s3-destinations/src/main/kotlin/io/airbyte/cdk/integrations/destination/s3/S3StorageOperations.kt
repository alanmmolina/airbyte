...
open class S3StorageOperations(
    ...
    fun getPartId(objectPath: String): String {
        ...
        objectNameByPrefix.computeIfAbsent(
            ...
            var objectListing = s3Client.listObjects(s3Config.bucketName, objectPath)
            if (objectListing != null) {
                objectList = objectList + objectListing.objectSummaries.map { it.key }
            }
            while (objectListing != null && objectListing.nextMarker != null) {
                objectListing =
